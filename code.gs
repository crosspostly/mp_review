...
function onOpen(e) {
  ...
  // 🎯 АВТОМАТИЧЕСКАЯ СИНХРОНИЗАЦИЯ ОБЩЕГО ТРИГГЕРА при открытии таблицы
  // Создает или удаляет общий триггер в зависимости от наличия активных магазинов
  if (!e || e.authMode !== ScriptApp.AuthMode.NONE) {
    // Только если пользователь авторизован
    try {
      const interval = getTriggerInterval();
      log(`[onOpen] Автоматическая синхронизация общего триггера (интервал: ${interval} мин)...`);
      const updated = ensureGlobalTrigger(interval);
      log(`[onOpen] Синхронизация завершена ${updated ? '(триггер обновлен)' : '(без изменений)'}`);
    } catch (e) {
      log(`[onOpen] ⚠️ Ошибка синхронизации триггера: ${e.message}`);
    }
  }
}
...
// ============ АВТОМАТИЧЕСКОЕ УПРАВЛЕНИЕ ОБЩИМ ТРИГГЕРОМ ============

/**
 * 🎯 АВТОМАТИЧЕСКОЕ УПРАВЛЕНИЕ ОБЩИМ ТРИГГЕРОМ processAllStores
 * Создает триггер если есть активные магазины, удаляет если все магазины неактивны
 * @param {number} intervalMinutes - Интервал запуска в минутах (по умолчанию 5)
 * @returns {boolean} true если триггер был создан/обновлен
 */
function ensureGlobalTrigger(intervalMinutes = 5) {
  const allStores = getStores();
  const activeStores = allStores.filter(s => s.isActive);
  
  // Проверяем, существует ли уже общий триггер
  const existingTriggers = ScriptApp.getProjectTriggers();
  const existingTrigger = existingTriggers.find(t => t.getHandlerFunction() === 'processAllStores');
  
  if (activeStores.length > 0) {
    // Есть активные магазины - нужен триггер
    
    if (existingTrigger) {
      log(`[Trigger] ℹ️ Общий триггер processAllStores уже существует (активных магазинов: ${activeStores.length})`);
      return false;
    }
    
    try {
      // Создаем новый общий триггер
      ScriptApp.newTrigger('processAllStores')
        .timeBased()
        .everyMinutes(intervalMinutes)
        .create();
      
      log(`[Trigger] ✅ Общий триггер processAllStores создан (интервал: ${intervalMinutes} мин, активных магазинов: ${activeStores.length})`);
      return true;
      
    } catch (e) {
      log(`[Trigger] ❌ ОШИБКА создания общего триггера: ${e.message}`);
      return false;
    }
    
  } else {
    // Нет активных магазинов - удаляем триггер если существует
    if (existingTrigger) {
      ScriptApp.deleteTrigger(existingTrigger);
      log(`[Trigger] 🗑️ Общий триггер processAllStores удален (нет активных магазинов)`);
      return true;
    } else {
      log(`[Trigger] ℹ️ Нет активных магазинов и нет триггера - ничего не делаем`);
      return false;
    }
  }\n}\n\n// ============ ИНДИВИДУАЛЬНЫЕ ТРИГГЕРЫ (УСТАРЕЛО) ============\n\n/**
 * ⚠️ УСТАРЕЛО: АВТОМАТИЧЕСКОЕ СОЗДАНИЕ/ОБНОВЛЕНИЕ триггера для магазина
 * Вызывается при сохранении или изменении статуса магазина
 * @param {Object} store - Конфигурация магазина
 * @param {number} intervalMinutes - Интервал запуска в минутах (по умолчанию 5)
 * @deprecated Используйте ensureGlobalTrigger() вместо этой функции
 */
